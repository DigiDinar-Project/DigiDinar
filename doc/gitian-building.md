...
*Setup instructions for a gitian build of DIGIDINAR Core using a VM or physical system.*
...
to the DIGIDINAR GitHub release page.
...
- [Building DIGIDINAR Core](#building-digidinar-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/digidinar-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/digidinar-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/digidinar-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/digidinar-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/digidinar-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/digidinar-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/digidinar-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@digidinar.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/digidinar-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/digidinar-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/digidinar-osx-*-build.assert
```
