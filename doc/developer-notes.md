...
- [Development guidelines](#development-guidelines)
    - [General DIGIDINAR Core](#general-digidinar-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
DIGIDINAR Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play DDRs (tDDR)" on the test network, if you
...
DIGIDINAR Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/digidinar-project/digidinar/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_digidinar
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_digidinar --log_level=test_suite
$ valgrind -v --leak-check=full src/digidinard -printtoconsole
```
...
# A coverage report will now be accessible at `./test_digidinar.coverage/index.html`.
```
...
- BitcoinMiner : Generates DDRs (if wallet is enabled).
...
However, in open source software such as DIGIDINAR Core, where everyone uses
...
pay attention to for reviewers of DIGIDINAR Core code.

General DIGIDINAR Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: DIGIDINAR Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against DIGIDINAR Core so that they can be integrated
...
file descriptors it will cause problems with DIGIDINAR's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:DIGIDINAR-Project/DIGIDINAR.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `digidinar-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `digidinar-cli` and the GUI debug console use this table to determine how to
