...
public:
    ...
    bool WriteZDDRSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZDDRSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZDDRSeed_deprecated(uint256& seed);
    bool EraseZDDRSeed();
    bool EraseZDDRSeed_deprecated();

    bool WriteZDDRCount(const uint32_t& nCount);
    bool ReadZDDRCount(uint32_t& nCount);
