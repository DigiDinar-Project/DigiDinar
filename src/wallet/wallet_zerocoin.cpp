    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZDDRSeed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZDDRSeed(hashSeed, ToByteVector(seed))) {
            return true;
        }
...
bool CWallet::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadZDDRSeed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZDDRSeed(hashSeed, vchSeed)) {
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    for (auto &it : mapMintsSelected) {
        ...
        CTxIn in;
        if(!ZDDRModule::createInput(in, mint, hashTxOut, spendVersion)) {
...
void CWallet::setZWallet(CzDDRWallet* zwallet)
{
    zwalletMain = zwallet;
    zddrTracker = std::unique_ptr<CzDDRTracker>(new CzDDRTracker(this));
}

CzDDRWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
