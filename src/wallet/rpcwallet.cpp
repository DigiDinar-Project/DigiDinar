...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new Digi Dinar address for receiving payments.\n"
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the current Digi Dinar address for receiving payments to this account.\n"
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new Digi Dinar address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Digi Dinar address");
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"todigidinaraddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a digidinar address.\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    for (const string& name_ : keys) {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid DIGIDINAR address: ")+name_);
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total DDR balance of the wallet\n"
...
UniValue multisend(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "multisend <DIGIDINAR Address> <percent>\n"
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zddrTracker->ListMints(true, fMatureOnly, true);
...
UniValue listzerocoinamounts(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zddrTracker->ListMints(true, true, true);
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    bool fMintChange = params[1].get_bool();        // Mint change to zDDR
    if (fMintChange && Params().NetworkID() != CBaseChainParams::REGTEST)
        throw JSONRPCError(RPC_WALLET_ERROR, "zDDR minting is DISABLED, cannot mint change");
    ...
    if (!ispublicspend && Params().NetworkID() != CBaseChainParams::REGTEST) {
        throw JSONRPCError(RPC_WALLET_ERROR, "zDDR old spend only available in regtest for tests purposes");
    }

    return DoZddrSpend(nAmount, fMintChange, fMinimizeChange, vMintsSelected, address_str, ispublicspend);
}
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    ...
    return DoZddrSpend(nAmount, false, true, vMintsSelected, address_str);
}


extern UniValue DoZddrSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool ispublicspend)
{
...
UniValue resetmintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    set<CMintMeta> setMints = zddrTracker->ListMints(false, false, true);
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzDDRTracker* zddrTracker = pwalletMain->zddrTracker.get();
    set<CMintMeta> setMints = zddrTracker->ListMints(false, false, false);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CzDDRTracker* zddrTracker = pwalletMain->zddrTracker.get();
    set<CMintMeta> setMints = zddrTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue createautomintaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                ...
                "\nResult\n"
                "\"address\"     (string) DIGIDINAR address for auto minting\n" +
...
UniValue spendrawzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 4 || params.size() > 5)
        throw runtime_error(
            ...
            "5. \"address\"          (string, optional) DIGIDINAR address to spend to. If not specified, spend to change add.\n"
...
UniValue clearspendcache(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 0)
        throw runtime_error(
            "clearspendcache\n"
            "\nClear the pre-computed zDDR spend cache, and database.\n" +
            HelpRequiringPassphrase() + "\n"
...

    {
        int nTries = 0;
        while (nTries < 100) {
            TRY_LOCK(zddrTracker->cs_spendcache, fLocked);
            if (fLocked) {
                if (zddrTracker->ClearSpendCache()) {
