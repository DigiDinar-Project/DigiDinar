...
CAmount CWallet::GetZerocoinBalance(bool fMatureOnly) const
{
    if (fMatureOnly) {
        ...
        CAmount nBalance = 0;
        vector<CMintMeta> vMints = zddrTracker->GetMints(true);
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        set<CMintMeta> setMints = zddrTracker->ListMints(true, true, true);
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount, bool fPrecompute)
{
    ...
    if ((GetBoolArg("-zddrstake", true) || fPrecompute) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        set<CMintMeta> setMints = zddrTracker->ListMints(true, true, fUpdate);
...
bool CWallet::MintableCoins()
{
    ...
    if (nBalance > 0) {
        ...
        CAmount nMinAmount = Params().StakeInputMinimal(chainActive.Height());
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 DDR collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    vector<pair<CScript, CAmount> > vecSend;
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 DDR collateral
...
string CWallet::GetUniqueWalletBackupName(bool fzddrAuto) const
{
...
void CWallet::ZDdrBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozddrbackup-%d.dat", i + 1);
    ...
    if(!GetArg("-zddrbackuppath", "").empty()) {
        filesystem::path customPath(GetArg("-zddrbackuppath", ""));
