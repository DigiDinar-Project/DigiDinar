...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zddrspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 DDR at the same time
...
enum ZerocoinSpendStatus {
    ZDDR_SPEND_OKAY = 0,                            // No error
    ZDDR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZDDR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZDDR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZDDR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZDDR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZDDR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZDDR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZDDR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZDDR_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZDDR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZDDR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZDDR_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZDDR_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZDDR_SPENT_USED_ZDDR = 14,                      // Coin has already been spend
    ZDDR_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZDDR_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZDdrBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZDDROutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzddrAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzDDRTracker> zddrTracker;
    ...
    void setZWallet(CzDDRWallet* zwallet)
    {
        zwalletMain = zwallet;
        zddrTracker = std::unique_ptr<CzDDRTracker>(new CzDDRTracker(strWalletFile));
    }

    CzDDRWallet* getZWallet() { return zwalletMain; }
    ...
    void setZDdrAutoBackups(bool fEnabled)
    {
    ...
    /** zDDR reset */
    boost::signals2::signal<void()> NotifyzDDRReset;
