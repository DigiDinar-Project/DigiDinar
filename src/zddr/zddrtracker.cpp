...
CzDDRTracker::CzDDRTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzDDRTracker::~CzDDRTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzDDRTracker::Init()
{
...
bool CzDDRTracker::Archive(CMintMeta& meta)
{
...
bool CzDDRTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzDDRTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzDDRTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzDDRTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CAmount CzDDRTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzDDRTracker::GetUnconfirmedBalance() const
{
...
bool CzDDRTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzDDRTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzDDRTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzDDRTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzDDRTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzDDRTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzDDRTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzDDRTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzDDRWallet* zDDRWallet)
{
    bool iszDDRWalletInitialized = (nullptr != zDDRWallet);
    CMintMeta meta;
    ...
    if (!iszDDRWalletInitialized)
        zDDRWallet = new CzDDRWallet(wallet);
    meta.isSeedCorrect = zDDRWallet->CheckSeed(dMint);
    if (!iszDDRWalletInitialized)
        delete zDDRWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzDDRTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDDRTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzDDRTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzDDRTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzDDRTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzDDRTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    ...
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzddr from DB\n", __func__, listDeterministicDB.size());
    }
