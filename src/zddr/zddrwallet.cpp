...
CzDDRWallet::CzDDRWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zddr seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDDRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDDRSeed_deprecated()) {
                    LogPrintf("%s: Updated zDDR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zddr seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zddr wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDDRWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDDRCount(nCountLastUsed);
    else if (!walletdb.ReadZDDRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDDRWallet::Lock()
{
    seedMaster.SetNull();
}

void CzDDRWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDDRWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDDR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDDRWallet::LoadMintPoolFromDB()
{
...
void CzDDRWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDDRWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDDRWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zddrTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDDRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDDR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    wallet->zddrTracker->Add(dMint, true);
...
void CzDDRWallet::SeedToZDDR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzDDRWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDDRWallet::GenerateDeterministicZDDR(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDDRWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDDRWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzDDRWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
