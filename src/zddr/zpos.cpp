...
bool CLegacyZDdrStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZDdrStake from non zc-spend");
    ...
    *this = CLegacyZDdrStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zddr stake origin", __func__);

    // All good
    return true;
}

CLegacyZDdrStake::CLegacyZDdrStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZDdrStake::GetIndexFrom()
{
...
CAmount CLegacyZDdrStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZDdrStake::GetUniqueness() const
{
...
bool CLegacyZDdrStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (nHeight < consensus.height_start_ZC_SerialsV2 || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zDDR stake block: height %d outside range", __func__, nHeight);
