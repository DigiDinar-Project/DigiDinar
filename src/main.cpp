...
#if defined(NDEBUG)
#error "DIGIDINAR cannot be compiled without assertions."
#endif
...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_Public_Spend_Enabled()) {
        ...
        if (v1Serial) {
            return error("%s : zDDR v1 serial spend not spendable, serial %s, tx %s\n", __func__,
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZDDRModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!ZDDRModule::validateInput(txin, prevOut, tx, ret)){
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
                    PublicCoinSpend publicSpend(params);
                    if (!ZDDRModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
            ...
            // Check that zDDR mints (if included) are not already known
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZDDRModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from DIGIDINAR
...
bool UpdateZDDRSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    // A one-time event where only the zDDR supply was off (due to serial duplication off-chain on main net)
...
bool CheckBlockForBlackListedAddresses(const CBlock& block, int nHeight)
{
   ...
   CBitcoinAddress addressExp1("D7xdmtyUFC9zvr6yQWpV1HKgCimdgcTn74");
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // DigiDinar Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t) Params().DigiDinarBadBlockTime()) && (block.nBits == (uint32_t) Params().DigiDinarBadBlocknBits())) {
            // accept DIGIDINAR block minted with incorrect proof of work threshold
            return true;
        }
        ...
        if (isBlockFromFork) {
            ...
            if(hasZDDRInputs){
                for (const CTxIn& zDdrInput : zDDRInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zDdrInput);
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // DIGIDINAR: We use certain sporks during IBD, so check to see if they are
