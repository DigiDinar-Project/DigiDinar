...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::DDR);
    ...
    if (settings.contains("nAnonymizeDigidinarAmount") || reset)
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in digidinar.cpp during shotdown to also remove the window positions
