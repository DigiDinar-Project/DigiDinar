...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl, bool fIncludeDelegations)
{
    ...
    Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered digidinar address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            Q_EMIT message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 DDR.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        Q_FOREACH (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal digidinar:URI (digidinar:XyZ...?message=example)
            {
...
bool WalletModel::createZddrSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZddr(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        CZerocoinSpendReceipt &receipt)
{
...
static void NotifyzDDRReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzDDRReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzDDRReset.connect(boost::bind(NotifyzDDRReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzDDRReset.disconnect(boost::bind(NotifyzDDRReset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose)
{
    ...
    if (address.IsStakingAddress())
        return error("Invalid DIGIDINAR address, cold staking address");
    CKeyID keyID;
...
{
    if (!address.IsValid())
        return error("Invalid DIGIDINAR address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from DIGIDINAR address");
...
{
    setMints.clear();
    setMints = pwalletMain->zddrTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
