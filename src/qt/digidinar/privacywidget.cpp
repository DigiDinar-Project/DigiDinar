...
#include "qt/digidinar/privacywidget.h"
#include "qt/digidinar/forms/ui_privacywidget.h"
...
PrivacyWidget::PrivacyWidget(DIGIDINARGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zDDR anonymizes your DDR by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zDDR or convert back to DDR"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 DDR ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zDDR");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zDDR");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zDDR");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zDDR");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zDDR");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zDDR");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zDDR");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zDDR");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zDDR Movements"));
    ...
    // Only Convert to DDR enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzDDR->setTitleClassAndText("btn-title-grey", "Total 0 zDDR");
    ui->btnTotalzDDR->setSubTitleClassAndText("text-subtitle", "Show denominations of zDDR owned.");
    ui->btnTotalzDDR->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select DDR outputs to mint into zDDR.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zDDR");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzDDR, &OptionButton::clicked, this, &PrivacyWidget::onTotalZddrClicked);
...
void PrivacyWidget::onMintSelected(bool isMint)
{
    QString btnText;
    if (isMint) {
        btnText = tr("Mint zDDR");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of DDR to mint into zDDR"));
    } else {
        btnText = tr("Convert back to DDR");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zDDR to convert back into DDR"));
    }
...
void PrivacyWidget::onTotalZddrClicked()
{
    bool isVisible = ui->layoutDenom->isVisible();
    if (!isVisible) {
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzDDR->setRightIconClass("btn-dropdown", true);
    } else {
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzDDR->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked()
{
    ...
    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zDDR is currently undergoing maintenance"));
        return;
    }
    ...
    if (!ctx.isValid()) {
        inform(tr("You need to unlock the wallet to be able to %1 zDDR").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value)
{
    ...
    if (!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)) {
        ...
        // Mint succeed
        inform(tr("zDDR minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value)
{
    ...
    if (!walletModel->convertBackZddr(
            value,
            selectedMints,
            receipt
    )) {
        ...
        // Spend succeed
        inform(tr("zDDR converted back to DDR"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply()
{
    ...
    ui->btnTotalzDDR->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
