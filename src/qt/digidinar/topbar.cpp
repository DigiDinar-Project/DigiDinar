...
#include "qt/digidinar/topbar.h"
#include "qt/digidinar/forms/ui_topbar.h"
#include "qt/digidinar/lockunlock.h"
#include "qt/digidinar/qtutils.h"
#include "qt/digidinar/receivedialog.h"
#include "qt/digidinar/loadingdialog.h"
...
TopBar::TopBar(DIGIDINARGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    std::initializer_list<QWidget*> lblTitles = {ui->labelTitle1, ui->labelTitleAvailablezDdr, ui->labelTitle3, ui->labelTitle4, ui->labelTitlePendingzDdr, ui->labelTitleImmaturezDdr};
    ...
    setCssProperty({ui->labelAmountTopDdr, ui->labelAmountTopzDdr}, "amount-small-topbar");
    setCssProperty({ui->labelAmountDdr, ui->labelAvailablezDdr}, "amount-topbar");
    setCssProperty({ui->labelPendingDdr, ui->labelPendingzDdr, ui->labelImmatureDdr, ui->labelImmaturezDdr}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("DIGIDINAR")
    updateDisplayUnit();
                            ...
                            const CAmount& delegatedBalance, const CAmount& coldStakedBalance)
{
    ...
    // DDR Total
    CAmount ddrAvailableBalance = balance;
    // zDDR Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalDdr = GUIUtil::formatBalance(ddrAvailableBalance, nDisplayUnit);
    QString totalzDdr = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);

    // DDR
    // Top
    ui->labelAmountTopDdr->setText(totalDdr);
    // Expanded
    ui->labelAmountDdr->setText(totalDdr);
    ui->labelPendingDdr->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelImmatureDdr->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));

    // Update display state and/or values for zDDR balances as necessary
    bool fHaveZerocoins = zerocoinBalance > 0;

    // Set visibility of zDDR label titles/values
    ui->typeSpacerTop->setVisible(fHaveZerocoins);
    ui->typeSpacerExpanded->setVisible(fHaveZerocoins);
    ui->labelAmountTopzDdr->setVisible(fHaveZerocoins);
    ui->zerocoinBalances->setVisible(fHaveZerocoins);

    // Top
    ui->labelAmountTopzDdr->setText(totalzDdr);
    // Expanded
    ui->labelAvailablezDdr->setText(totalzDdr);
    ui->labelPendingzDdr->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));
    ui->labelImmaturezDdr->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
