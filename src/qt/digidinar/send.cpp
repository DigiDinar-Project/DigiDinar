...
#include "qt/digidinar/send.h"
#include "qt/digidinar/forms/ui_send.h"
#include "qt/digidinar/addnewcontactdialog.h"
#include "qt/digidinar/qtutils.h"
#include "qt/digidinar/sendchangeaddressdialog.h"
#include "qt/digidinar/optionbutton.h"
#include "qt/digidinar/sendconfirmdialog.h"
#include "qt/digidinar/myaddressrow.h"
#include "qt/digidinar/guitransactionsutils.h"
...
SendWidget::SendWidget(DIGIDINARGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("DDR");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zDDR");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (DDR) or private coins (zDDR)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("DIGIDINAR address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 DDR");
    ...
    setCssProperty(coinIcon, "coin-icon-ddr");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onDDRSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onDDRSelected(false);});
...
void SendWidget::refreshView()
{
    ...
    ui->pushButtonSave->setText(isChecked ? tr("Send DDR") : tr("Send zDDR"));
...
void SendWidget::refreshAmounts()
{
    ...
    bool isZddr = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZddr));
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend DDR (not zDDR)
    const bool isZddr = ui->pushRight->isChecked();
    const bool isCControl = CoinControlDialog::coinControl->HasSelected();
    const bool hasDel = cachedDelegatedBalance > 0;

    const bool showCheckBox = !isZddr && !isCControl && hasDel;
    ...
    if (showCheckBox)
        ui->checkBoxDelegations->setToolTip(
                tr("Possibly spend coins delegated for cold-staking (currently available: %1").arg(
                        GUIUtil::formatBalance(cachedDelegatedBalance, nDisplayUnit, isZddr))
        );
}
...
void SendWidget::onSendClicked()
{
    ...
    bool sendDdr = ui->pushLeft->isChecked();
    ...
    if ((sendDdr) ? send(recipients) : sendZddr(recipients)) {
...
bool SendWidget::sendZddr(QList<SendCoinsRecipient> recipients)
{
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        Q_EMIT message(tr("Spend Zerocoin"), tr("zDDR is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    if (walletModel->sendZddr(
        ...
        inform(tr("zDDR transaction sent!"));
        ...
        if (receipt.GetStatus() == ZDDR_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zDDR require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().GetConsensus().ZC_MaxSpendsPerTx; // Maximum possible spends for one zDDR transaction
        ...
        Q_EMIT message("zDDR transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked()
{
    if (isDDR) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any DDR to select."));
        }
    } else {
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            zDdrControl->exec();
            ui->btnCoinControl->setActive(!ZDdrControlDialog::setSelectedMints.empty());
            zDdrControl->deleteLater();
        } else {
            inform(tr("You don't have any zDDR in your balance to select."));
        }
    }
}
...
void SendWidget::onDDRSelected(bool _isDDR)
{
    isDDR = _isDDR;
    setCssProperty(coinIcon, _isDDR ? "coin-icon-ddr" : "coin-icon-zddr");
    refreshView();
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CBitcoinAddress ddrAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(ddrAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(ddrAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
