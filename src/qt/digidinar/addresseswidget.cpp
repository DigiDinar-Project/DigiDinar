...
#include "qt/digidinar/forms/ui_addresseswidget.h"
#include "qt/digidinar/addresslabelrow.h"
#include "qt/digidinar/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(DIGIDINARGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter DIGIDINAR address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress ddrAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(ddrAdd.Get(), label.toUtf8().constData(),
                ddrAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
