...
#include "qt/digidinar/forms/ui_snackbar.h"
#include "qt/digidinar/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(DIGIDINARGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &DIGIDINARGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
