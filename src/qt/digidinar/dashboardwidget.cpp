...
#include "qt/digidinar/dashboardwidget.h"
#include "qt/digidinar/forms/ui_dashboardwidget.h"
#include "qt/digidinar/sendconfirmdialog.h"
#include "qt/digidinar/txrow.h"
...
DashboardWidget::DashboardWidget(DIGIDINARGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of DDR and zDDR staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareDdr, "square-chart-ddr");
    setCssProperty(ui->labelSquarezDdr, "square-chart-zddr");
    setCssProperty(ui->labelDdr, "text-chart-ddr");
    setCssProperty(ui->labelZddr, "text-chart-zddr");
    ...
    ui->labelAmountZddr->setText("0 zDDR");
    ui->labelAmountDdr->setText("0 DDR");
    setCssProperty(ui->labelAmountDdr, "text-stake-ddr-disable");
    setCssProperty(ui->labelAmountZddr, "text-stake-zddr-disable");
    ...
    ui->btnHowTo->setText(tr("How to get DDR or zDDR"));
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &DIGIDINARGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &DIGIDINARGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &DIGIDINARGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isDdr = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZDDR;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair DDR, zDDR
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            ddr = (pair.first != 0) ? pair.first / 100000000 : 0;
            zddr = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalDdr += pair.first;
            chartData->totalZddr += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesDdr.append(ddr);
        chartData->valueszDdr.append(zddr);

        int max = std::max(ddr, zddr);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    // init sets
    set0 = new QBarSet("DDR");
    set1 = new QBarSet("zDDR");
    ...
    set0->append(chartData->valuesDdr);
    set1->append(chartData->valueszDdr);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalDdr > 0 || chartData->totalZddr > 0) {
        setCssProperty(ui->labelAmountDdr, "text-stake-ddr");
        setCssProperty(ui->labelAmountZddr, "text-stake-zddr");
    } else {
        setCssProperty(ui->labelAmountDdr, "text-stake-ddr-disable");
        setCssProperty(ui->labelAmountZddr, "text-stake-zddr-disable");
    }
    forceUpdateStyle({ui->labelAmountDdr, ui->labelAmountZddr});
    ui->labelAmountDdr->setText(GUIUtil::formatBalance(chartData->totalDdr, nDisplayUnit));
    ui->labelAmountZddr->setText(GUIUtil::formatBalance(chartData->totalZddr, nDisplayUnit, true));
