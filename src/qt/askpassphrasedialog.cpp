...
void AskPassphraseDialog::accept()
{
    ...
    case Mode::Encrypt: {
        ...
        bool ret = openStandardDialog(
                ...
                " <b>" + tr("LOSE ALL OF YOUR DDR") + "</b>!<br><br>" + tr("Are you sure you wish to encrypt your wallet?"),
                tr("ENCRYPT"), tr("CANCEL")
        );
        if (ret) {
            if (newpass1 == newpass2) {
                newpassCache = newpass1;
                DIGIDINARGUI* window = static_cast<DIGIDINARGUI*>(parentWidget());
...
{
    DIGIDINARGUI* gui = static_cast<DIGIDINARGUI*>(parentWidget());
...
{
    hide();
    static_cast<DIGIDINARGUI*>(parentWidget())->showHide(true);
    openStandardDialog(
            tr("Wallet encrypted"),
            "<qt>" +
