...
bool CZDdrControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZDdrControlDialog::COLUMN_DENOMINATION || column == ZDdrControlDialog::COLUMN_VERSION || column == ZDdrControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZDdrControlDialog::ZDdrControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZDdrControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zDDR Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZDdr->setProperty("cssClass", "text-main-purple");
    ui->labelZDdr_int->setProperty("cssClass", "text-main-purple");
    ...
    // click on checkbox
    connect(ui->treeWidget, &QTreeWidget::itemChanged, this, &ZDdrControlDialog::updateSelection);
    // push select/deselect all button
    connect(ui->pushButtonAll, &QPushButton::clicked, this, &ZDdrControlDialog::ButtonAllClicked);
}

ZDdrControlDialog::~ZDdrControlDialog()
{
    delete ui;
}

void ZDdrControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDdrControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZDdrControlWidgetItem *itemMint = new CZDdrControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < nRequiredConfs)
                ...
                strReason = "Your wallet is locked. Impossible to spend zDDR.";
            else if (!mint.isSeedCorrect)
                strReason = "The zDDR seed used to mint this zDDR is not the same as currently hold in the wallet";
            else
...
void ZDdrControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDdrControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZDdr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZDdrControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZDdrControlDialog::GetSelectedMints()
{
...
void ZDdrControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
