...
// zDDRStake can take two forms
// 1) the stake candidate, which is a zcmint that is attempted to be staked
// 2) a staked zddr, which is a zcspend that has successfully staked
class CZDdrStake : public CStakeInput
{
private:
    uint32_t nChecksum;
    bool fMint;
...
public:
    explicit CZDdrStake(libzerocoin::CoinDenomination denom, const uint256& hashSerial)
    {
        this->denom = denom;
    ...
    explicit CZDdrStake(const libzerocoin::CoinSpend& spend);
...
class CDdrStake : public CStakeInput
{
private:
    CTransaction txFrom;
