...
/**
 * DIGIDINAR RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZddrSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzddrseed(const UniValue& params, bool fHelp);
extern UniValue getzddrseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzddr(const UniValue& params, bool fHelp);
extern UniValue dzddrstate(const UniValue& params, bool fHelp);
