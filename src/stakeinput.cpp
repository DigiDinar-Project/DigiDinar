...
bool CDdrStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CDdrStake from zerocoin spend");
...
bool CDdrStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CDdrStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CDdrStake::GetTxOutFrom(CTxOut& out) const
{
...
CAmount CDdrStake::GetValue() const
{
...
CDataStream CDdrStake::GetUniqueness() const
{
...
bool CDdrStake::ContextCheck(int nHeight, uint32_t nTime)
{
