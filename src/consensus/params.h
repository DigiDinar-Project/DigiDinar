...
struct Params {
    ...
    unsigned int nDigidinarBadBlockBits;
            ...
            const int utxoFromBlockHeight, const uint32_t utxoFromBlockTime) const
    {
        ...
        if (contextHeight - utxoFromBlockHeight >= nStakeMinDepth)
            return true;
        
        return (contextHeight - utxoFromBlockHeight >= 150) && (utxoFromBlockTime + nStakeMinAge <= contextTime);
    }
