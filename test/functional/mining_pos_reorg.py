...
class ReorgStakeTest(DigidinarTestFramework):
    ...
    def check_money_supply(self, expected_ddr, expected_zddr):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected DDR and zDDR supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_ddr))
            for denom in node['zDDRsupply']:
                assert_equal(node['zDDRsupply'][denom], DecimalAmt(expected_zddr[denom]))
    ...
    def run_test(self):
        ...
        # Check DDR and zDDR supply at the beginning
        # ------------------------------------------
        # zDDR supply: 2 coins for each denomination
        expected_zddr_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # DDR supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zddr_supply)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zddr",
        ...
        # Verify that DDR and zDDR supplies were properly updated after the spends and reorgs
        self.log.info("Check DDR and zDDR supply...")
        ...
        expected_zddr_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zddr_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zddr_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zddr_supply)
