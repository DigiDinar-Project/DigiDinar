...
class ZerocoinSpendTest(DigidinarTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zddr_bal, ddr_bal):
            zddr_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zddr_bal)
            ddr_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], ddr_bal)
            return zddr_bal, ddr_bal
        ...
        zddr_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zddr_balance, 6666)
        ...
        zddr_balance, balance = check_balances(denom_2, zddr_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zddr_balance, balance = check_balances(denom_3, zddr_balance, balance)
        ...
        _, _ = check_balances(denom_1, zddr_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
